...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope dgx-spark-cli
...
scope:** the default personal scope is **private** (`--scope dgx-spark-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `dgx-spark-cli`). Private records are served only to a recall in the same
