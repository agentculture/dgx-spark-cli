...
type: command
description: >
  Cross-repo + mesh communication from dgx-spark-cli: file tracked GitHub issues
  ...
  the next step lives outside dgx-spark-cli (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- dgx-spark-cli (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-dgx-spark-cli
